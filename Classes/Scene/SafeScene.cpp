...
void SafeScene::update(float delta)
{
	...
	// box
	box->isboxwithknight(knight);
	box->ispropswithknight(knight);

	//store
	store->checkstore(510, 790, knight);
	store->isstorewithknight(knight);
	store->isgoodswithknight(knight);

	//statue
	statue->isstatuewithknight(knight);
}
...
bool SafeScene::init()
{
	...
	//store
	store = Store::create();
	store->setPosition(Vec2(visibleSize.width / 2 + 150, visibleSize.height / 2 + 150));
	store->setScale(0.5);
	this->addChild(store, 3);
	store->goods[store->goodstype]->setPosition(Vec2(visibleSize.width / 2 + 150, visibleSize.height / 2 + 150));
	this->addChild(store->goods[store->goodstype], 4);	
	store->goods[store->goodstype]->setVisible(false);

	//statue
	statue = Statue::create();
	statue->setScale(0.5);
	statue->setPosition(Vec2(visibleSize.width / 2 - 100, visibleSize.height / 2));
	this->addChild(statue, 3);
