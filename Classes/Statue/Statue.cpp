...
bool Statue::isstatuewithknight(Knight* knight)
{
	if (this->getBoundingBox().intersectsRect(knight->getBoundingBox()) && isopened == false)
	{
		...
		{
			statueLab[bufftype] = FloatText::create("Pay " + Value(this->money_spend[bufftype]).asString() + " money to worship the statue", Vec2(getContentSize().width / 2, getContentSize().height + 10), 2, 40);
